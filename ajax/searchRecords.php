<?php
	...
	if($_POST['filter']=="ALL TIME")
	{
		$query = "SELECT * FROM tbl_orders";
	}
	else if ($_POST['filter']=="LAST 7 DAYS") {
		$query = "SELECT * FROM tbl_orders WHERE date_of_order BETWEEN DATE(NOW()) - INTERVAL 7 DAY AND DATE(NOW())
        ORDER BY date_of_order DESC;";
	}
	else if($_POST['filter']=="TODAY")
	{
		$query="SELECT * FROM `tbl_orders` WHERE date(date_of_order)=CURDATE()";
	}
